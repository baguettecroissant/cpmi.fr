...
export function DepartmentBreadcrumb({ city, prestationSlug, prestationLabel }: DepartmentBreadcrumbProps) {
    ...
    return (
        <nav aria-label="Fil d'ariane" className="text-sm text-slate-500 mb-6 flex items-center flex-wrap gap-1">
            ...
            <Link href={`/${prestationSlug}`} className="hover:text-blue-600">
                {prestationLabel}
            </Link>
            <ChevronRight className="h-3 w-3 text-slate-300" />

            <Link href={`/annuaire/${deptSlug}`} className="hover:text-blue-600">
