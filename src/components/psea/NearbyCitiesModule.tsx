...
export function NearbyCitiesModule({ city, prestationSlug, prestationLabel }: NearbyCitiesModuleProps) {
        ...
        <section className="bg-white border-t border-slate-100 py-12">
            <div className="container mx-auto px-4">
                <h3 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
                    ...
                    {prestationLabel} à proximité de {city.name}
                </h3>
                <nav aria-label="Villes à proximité" className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {nearby.map((c) => (
                        <Link
                            key={c.slug}
                            href={`/${prestationSlug}/${c.slug}`}
                            className="text-slate-600 hover:text-blue-600 hover:bg-blue-50 p-2 rounded-md transition-colors text-sm truncate block"
                            title={`${prestationLabel} à ${c.name} (${c.zip})`}
                        >
